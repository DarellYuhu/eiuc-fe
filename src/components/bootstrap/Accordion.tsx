...
export const AccordionItem = forwardRef<HTMLDivElement, IAccordionItemProps>(
  (
    { id, icon, title, children, tag, headerTag, overWriteColor, ...props },
    ref,
  ) => {
...
>(
  (
    { tag, id, activeItemId, children, shadow, color, isFlush, className },
    ref,
  ) => {
    const [activeItem, setActiveItem] = useState<TActiveItemId>(
      activeItemId === false
        ? null
        : activeItemId ||
            (Array.isArray(children) && children?.flat()[0].props.id),
    );
